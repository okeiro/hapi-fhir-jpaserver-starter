...
public class JSONDataReader {
	...
	public static JSONObject getJsonData(DocumentReference documentReference) {
		if (documentReference != null
				&& documentReference.hasContent()
				&& !documentReference.getContent().isEmpty()) {
		...
		if (!content.hasAttachment() && !content.getAttachment().hasData()) {
			throw new InvalidRequestException(
					"DocumentReference.content[0].attachment.data is required for this implementation !");
		} else if (!supportedMimeType.equals(content.getAttachment().getContentType())) {
			throw new InvalidRequestException(
					"DocumentReference.content[0].attachment.contentType must be \"application/json\" for this implementation !");
		}
